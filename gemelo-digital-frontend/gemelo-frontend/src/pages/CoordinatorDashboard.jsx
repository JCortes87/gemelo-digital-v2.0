...
export default function CoordinatorDashboard({ onClose }) {
      ...
      async function worker() {
        while (queue.length > 0 && alive) {
          ...
          try {
            ...
            if (alive)
              setCourseMetrics((prev) => ({ ...prev, [c.id]: metric }));
          } catch {
            if (alive)
  ...
  const sortedCourses = useMemo(() => {
    ...
    withMetrics.sort((a, b) => {
      ...
      if (sortBy === "atRiskPct")
        return (b.m.atRiskPct ?? -1) - (a.m.atRiskPct ?? -1);
      return 0;
    });
    return withMetrics;
  ...
  const filteredTotals = useMemo(() => {
    ...
    const totalStudents = metrics.reduce(
      (a, m) => a + (m.totalStudents || 0),
      0,
    );
  ...
  const metricsLoaded = Object.values(courseMetrics).filter(
    (m) => !m?.error,
  ).length;
    ...
    >
      ...
      <main style={{ padding: "24px 28px", maxWidth: 1200, margin: "0 auto" }}>
        ...
        {!hasSearched && (
          ...
          >
            <div
              style={{ fontSize: 40, opacity: 0.35, marginBottom: 12 }}
            ></div>
            <div
              style={{
                fontSize: 15,
            ...
            <div style={{ fontSize: 12, maxWidth: 380, margin: "0 auto" }}>
              Elige el período académico, tipo de programa o nombre de
              asignatura y haz clic en <strong>Buscar cursos</strong> para
              cargar los datos.
            </div>
          </div>
        )}
              ...
              >
                ...
                <div className="kpi-card">
                  ...
                  <div
                    style={{
                      fontSize: 32,
                      fontWeight: 900,
                      color: colorForPct(filteredTotals.avgGrade, null),
                      marginTop: 4,
                    }}
                  >
                    {filteredTotals.avgGrade != null
                      ? fmtGrade10FromPct(filteredTotals.avgGrade)
                      : "—"}
                    <span
                      style={{
                        fontSize: 14,
                ...
                <div className="kpi-card">
                  ...
                  >
                    ...
                    >
                      {filteredTotals.totalStudents > 0 &&
                        ` · ${((filteredTotals.atRiskCount / filteredTotals.totalStudents) * 100).toFixed(0)}%`}
                    </span>
                  </div>
                </div>
              </div>
                  ...
                  >
                    ...
                    {metricsLoaded < sortedCourses.length && (
                      <span style={{ marginLeft: 6, color: "var(--brand)" }}>
                        · cargando métricas {metricsLoaded}/
                        {sortedCourses.length}...
                      </span>
                    )}
                  </span>
                      ...
                      <tr>
                        ...
                        >
                          ...
                          <div style={{ fontSize: 13 }}>
                            Sin cursos encontrados con estos filtros.
                          </div>
                        </td>
                      </tr>
                    )}
                    {sortedCourses.map(
                      ({ id, name, code, m, _semester, _category }) => {
                        const avgColor =
                          m.avgPct != null
                            ? colorForPct(m.avgPct, null)
                            : "var(--muted)";
                        const riskColor =
                          m.atRiskPct == null
                            ? "var(--muted)"
                        ...
                        return (
                          ...
                          >
                            ...
                            >
                              {m.atRiskCount != null
                                ? `${m.atRiskCount} (${m.atRiskPct?.toFixed(0)}%)`
                                : "—"}
                            </td>
                            <td
                              style={{
