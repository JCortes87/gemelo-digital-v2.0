...
// ─────────────────────────────────────────────
const CircularRing = React.memo(function CircularRing({
  pct,
  size = 80,
  stroke = 8,
  color,
  ...
  return (
    <div
      ...
      aria-label={
        sublabel
          ? `${sublabel}: ${Math.round(pctClamped)}%`
          : `Progreso ${Math.round(pctClamped)}%`
      }
    >
      <svg
        width={size}
      ...
      >
        ...
        >
          {label ?? `${Math.round(pctClamped)}%`}
        </span>
        {sublabel && (
          <span
            style={{
              fontSize: Math.round(textSize * 0.55),
              fontWeight: 700,
              color: "var(--muted)",
              marginTop: 1,
            }}
    ...
    >
      ...
      >
        ...
        <div style={{ fontSize: 12, color: "var(--muted)", marginBottom: 16 }}>
          Define los porcentajes a partir de los cuales un estudiante se
          considera <b>en observación</b> o <b>crítico</b>. Se aplica solo a
          este curso.
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <label style={{ display: "block" }}>
            ...
            >
              <span>Crítico (&lt; %)</span>
              <span style={{ fontFamily: "var(--font-mono)" }}>{crit}%</span>
            </div>
            <input
              type="range"
              min={0}
          ...
          <label style={{ display: "block" }}>
            ...
            >
              ...
              <span style={{ fontFamily: "var(--font-mono)" }}>{watch}%</span>
            </div>
            <input
              type="range"
              min={0}
        ...
        <div style={{ fontSize: 10, color: "var(--muted)", marginTop: 12 }}>
          Por defecto del sistema: crítico {base?.critical ?? 50}% · observación{" "}
          {base?.watch ?? 70}%
        </div>
...
const _tooltipRoot = (() => {
  ...
  if (!el) {
    ...
    el.style.cssText =
      "position:fixed;top:0;left:0;width:0;height:0;overflow:visible;z-index:999999;pointer-events:none;";
...
const ONBOARDING_STEPS = [
  {
    ...
    highlight: null,
    voice: (name) =>
      `Bienvenido a G.D${name ? ", " + name : ""}. Tu asistente académico inteligente para el seguimiento de tus estudiantes.`,
  },
  {
    id: "dashboard",
    ...
    highlight: "dashboard",
    voice: () =>
      "El Dashboard te da el panorama completo de tu curso. Aquí ves el promedio, los estudiantes en riesgo y el cumplimiento evaluativo.",
  },
  {
    id: "priority",
    ...
    highlight: "priority",
    voice: () =>
      "La sección de estudiantes prioritarios te muestra quiénes necesitan atención urgente, con nota crítica o pendientes sin calificar.",
  },
  {
    id: "calendar",
    ...
    highlight: null,
    voice: () =>
      "El Calendario de entregas muestra todas las fechas del curso. Puedes ver el estado de cada actividad por estudiante.",
  },
  {
    id: "routes",
    ...
    highlight: "routes",
    voice: () =>
      "Las Rutas de atención te indican qué acción tomar con cada estudiante, desde activar evidencias hasta planes de recuperación.",
  },
  {
    id: "ai",
    ...
    highlight: "assistant",
    voice: () =>
      "El Asistente de Inteligencia Artificial responde tus preguntas en lenguaje natural. Puedes escribir o usar el micrófono para consultar el estado de tu curso.",
  },
  {
    id: "courses",
    ...
    highlight: null,
    voice: () =>
      "Puedes cambiar entre tus cursos en cualquier momento usando el botón Mis cursos. Si tienes doble rol, elige tu vista desde la pantalla de inicio. ¡Listo para comenzar!",
  },
];
        ...
        <div style={{ textAlign: "center", marginBottom: 10 }}>
          <span
            style={{
              ...
              border:
                "1px solid " +
                (TAG_COLORS[current.tag] || "var(--brand)") +
                "40",
            }}
          >
            {current.tag}
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Divider */}
        <div
          style={{ height: 1, background: "var(--border)", margin: "0 0 28px" }}
        />
        ...
        >
          Para acceder a tu tablero, inicia sesión con tu cuenta CESA de
          Brightspace. Serás redirigido a Microsoft para autenticarte.
        </p>
        ...
        >
          Solo los instructores con cursos activos en Brightspace pueden
          acceder. Si tienes problemas, contacta a soporte CESA.
        </p>
...
function CesaLoader({
  title = "CESA · G.D V.260428",
  subtitle = "Cargando tablero...",
}) {
  React.useEffect(() => {
    injectStyles();
  }, []);
  ...
  return (
    <div className="cesa-loader-wrap">
      <div className="cesa-loader-card">
        ...
        <div className="cesa-loader-foot">
          Conectando con Brightspace y consolidando evidencias académicas…
        </div>
      </div>
    </div>
  );
    ...
    <div style={{ marginTop: 8 }}>
      ...
      {open && (
        ...
        >
          ...
          >
            Vincula estas actividades a una rúbrica con RA en Brightspace para
            incluirlas en el análisis de competencias.
          </div>
        </div>
      )}
    </div>
...
function AlertsPanel({ alerts }) {
  ...
  const sevRank = (s) => {
    ...
    if (x === "en desarrollo" || x === "en seguimiento" || x === "observacion")
      return 1;
    return 2;
  };
  ...
  const countBySev = (sev) =>
    sorted.filter((x) => normStatus(x.severity) === sev).length;
  const cCrit = countBySev("critico");
  const cObs = sorted.filter((x) =>
    ["en desarrollo", "en seguimiento", "observacion"].includes(
      normStatus(x.severity),
    ),
  ).length;
        ...
        >
          ...
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
            {cCrit > 0 && (
              <span
                className="badge"
                style={{ background: "var(--critical-bg)", color: "#B42318" }}
              >
                <span
                  style={{
                    width: 6,
            ...
            {cObs > 0 && (
              <span
                className="badge"
                style={{ background: "var(--watch-bg)", color: "#9A3412" }}
              >
                <span
                  style={{
                    width: 6,
            ...
            {cSol > 0 && (
              <span
                className="badge"
                style={{ background: "var(--ok-bg)", color: "#1B5E20" }}
              >
                <span
                  style={{
                    width: 6,
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Content */}
        <div style={{ padding: "16px 20px 28px", flex: 1 }}>{children}</div>
      </div>
    </div>
  );
}
...
function ProjectionBlock({ projection, thresholds }) {
  ...
  if (projection.isFinal) {
    return (
      <Card
        ...
        right={<span className="tag">Cobertura 100%</span>}
      >
        <Stat
          label="Nota final"
          value={fmtGrade10FromPct(projection.finalPct)}
          valueColor={colorForPct(projection.finalPct, thresholds)}
        />
  ...
  const scenarioMeta = {
    risk: {
      label: "Escenario riesgo",
      sub: "si el resto baja",
      cls: "scenario-risk",
      icon: "",
    },
    base: {
      label: "Escenario base",
      sub: "desempeño actual",
      cls: "scenario-base",
      icon: "",
    },
    improve: {
      label: "Escenario mejora",
      sub: "si el resto sube",
      cls: "scenario-improve",
      icon: "",
    },
  };
  ...
  return (
    <Card
      title="Proyección de nota final"
      right={
        <span className="tag">{fmtPct(projection.coveragePct)} calificado</span>
      }
    >
      <div
        style={{
            ...
            <div key={s.id} className={`scenario-card ${meta.cls}`}>
              ...
              <div
                style={{
                  ...
                  fontWeight: 900,
                  letterSpacing: "-0.02em",
                  color: colorForPct(s.projectedFinalPct, thresholds),
                }}
              >
    ...
    >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <div>
            <div style={{ fontSize: 12, fontWeight: 800, color: "#9A3412" }}>
              {unlinked.length} asignación{unlinked.length !== 1 ? "es" : ""}{" "}
              calificada{unlinked.length !== 1 ? "s" : ""} sin Resultado de
              Aprendizaje
            </div>
            <div style={{ fontSize: 11, color: "var(--muted)" }}>
              Estas evidencias tienen nota pero no están vinculadas a ningún RA
              en la rúbrica
            </div>
          </div>
        </div>
      ...
      {open && (
        ...
        >
              ...
              >
                ...
                <span
                  style={{
                    ...
                    fontSize: 13,
                    fontWeight: 800,
                    color: colorForPct(e.scorePct, null),
                  }}
                >
          ...
          >
            Para que aparezcan en el análisis de RA, vincula estas asignaciones
            a una rúbrica con criterios mapeados en Brightspace.
          </div>
        </div>
      )}
    </div>
          ...
          {missing.length > 0 && (
            <div style={{ marginTop: items.length > 0 ? 12 : 0 }}>
              ...
              <div style={{ fontSize: 12, color: "var(--muted)" }}>
                Ítems sin valor visible para el estudiante. Revisar
...
function EvidencesTimeline({ evidences, thresholds }) {
  const list = Array.isArray(evidences)
    ? evidences.filter((e) => e.scorePct !== null && e.scorePct !== undefined)
    : [];
  if (!list.length) return null;
  const [open, setOpen] = React.useState(false);
          ...
          <div style={{ width: "100%", height: 160 }}>
            <ResponsiveContainer>
              <LineChart data={chartData} margin={{ left: -10, right: 10 }}>
                ...
                />
                <Tooltip
                  formatter={(v) => [`${Number(v).toFixed(1)}%`, "Desempeño"]}
                />
                <ReferenceLine
                  y={Number(thresholds?.watch || 70)}
                  stroke={COLORS.watch}
                  strokeDasharray="4 4"
                  label={{ value: "70%", fill: COLORS.watch, fontSize: 10 }}
                />
                <ReferenceLine
                  y={Number(thresholds?.critical || 50)}
                  stroke={COLORS.critical}
                  strokeDasharray="4 4"
                  label={{ value: "50%", fill: COLORS.critical, fontSize: 10 }}
                />
                <Line
                  type="monotone"
                  dataKey="pct"
...
function VoiceAssistant({
  ...
  const [msgs, setMsgs] = React.useState(() => [
    {
      ...
      role: "bot",
      fromVoice: false,
      text: `Listo. Tengo cargados los datos de <strong>${courseInfo?.Name || "este curso"}</strong>. Puedo analizar riesgo, evidencias y desempeño por RA. Escríbeme o usa el micrófono.`,
    },
  ]);
  ...
  const withGrades = (Array.isArray(studentRows) ? studentRows : []).filter(
    (s) => s.currentPerformancePct != null,
  );
  const avg = withGrades.length
    ? (
        withGrades.reduce(
          (a, s) => a + Number(s.currentPerformancePct) / 10,
          0,
        ) / withGrades.length
      ).toFixed(2)
    : null;
  const altos = (Array.isArray(studentRows) ? studentRows : []).filter(
    (s) => computeRiskFromPct(s.currentPerformancePct) === "alto",
  );
  const medios = (Array.isArray(studentRows) ? studentRows : []).filter(
    (s) => computeRiskFromPct(s.currentPerformancePct) === "medio",
  );
  const zeros = (Array.isArray(studentRows) ? studentRows : []).filter(
    (s) => s.currentPerformancePct == null,
  );
  ...
  const SUGGESTION_BANK = [
    ...
    { icon: "", label: "¿Hay estudiantes sin nota?" },
    ...
    { icon: "", label: "¿Qué rutas de intervención hay?" },
  ...
  function processCmd(cmd) {
    ...
    if (
      c.includes("actividad reciente") ||
      c.includes("sin actividad") ||
      c.includes("inactiv")
    ) {
      if (!zeros.length)
        return "Todos los estudiantes han tenido actividad registrada.";
      return `Sin actividad registrada: <strong>${zeros.length}</strong>:<br>${zeros
        .slice(0, 5)
        .map((s) => `‣ ${s.displayName.split(",")[0]}`)
        .join(
          "<br>",
        )}${zeros.length > 5 ? `<br>… y ${zeros.length - 5} más` : ""}`;
    }
    ...
    if (
      c.includes("que ra") ||
      c.includes("cual ra") ||
      c.includes("ra critico") ||
      c.includes("ra mas") ||
      c.includes("resultado de aprendizaje") ||
      c.includes("resultados de aprendizaje") ||
      c.includes("competencia") ||
      (c.includes("ra") &&
        (c.includes("critico") || c.includes("bajo") || c.includes("peor")))
    ) {
      const ras = Array.isArray(raDashboard?.ras)
        ? raDashboard.ras.filter((r) => r.studentsWithData > 0)
        : [];
      if (!ras.length)
        return "Sin datos de RA aún. Se requieren evaluaciones con rúbricas calificadas.";
    ...
    if (
      c.includes("debajo de 5") ||
      c.includes("menor a 5") ||
      c.includes("menor de 5") ||
      c.includes("5.0") ||
      c.includes("reprobado") ||
      (c.includes("cuantos") && c.includes("5"))
    ) {
      const rep = withGrades.filter((s) => s.currentPerformancePct / 10 < 5);
      return `Con nota menor a 5.0: <strong>${rep.length} de ${n}</strong>.${
        rep.length
          ? "<br>" +
            rep
              .slice(0, 4)
              .map(
                (s) =>
                  `‣ ${s.displayName.split(",")[0]} (${fmtGrade10FromPct(s.currentPerformancePct)})`,
              )
              .join("<br>")
          : ""
      }`;
    ...
    if (
      (c.includes("nota") &&
        (c.includes("mas baja") ||
          c.includes("baja") ||
          c.includes("peor") ||
          c.includes("menor nota"))) ||
      c.includes("nota minima") ||
      (c.includes("quien") && c.includes("baj"))
    ) {
      const worst = [...withGrades].sort(
        (a, b) => a.currentPerformancePct - b.currentPerformancePct,
      )[0];
    ...
    if (
      c.includes("riesgo alto") ||
      c.includes("alto riesgo") ||
      (c.includes("riesgo") &&
        (c.includes("quienes") ||
          c.includes("quién") ||
          c.includes("quienes estan")))
    ) {
      if (!altos.length) return "Ningún estudiante en riesgo alto actualmente.";
      return `Riesgo alto (${altos.length}):<br>${altos
        .slice(0, 6)
        .map(
          (s) =>
            `‣ ${s.displayName.split(",")[0]} — ${fmtGrade10FromPct(s.currentPerformancePct)}`,
        ...
        .join(
          "<br>",
        )}${altos.length > 6 ? `<br>… y ${altos.length - 6} más` : ""}`;
    }

    // ── Riesgo medio
    if (c.includes("riesgo medio") || c.includes("medio riesgo")) {
      if (!medios.length)
        return "Ningún estudiante en riesgo medio actualmente.";
      return `Riesgo medio (${medios.length}):<br>${medios
        .slice(0, 5)
        .map(
          (s) =>
            `‣ ${s.displayName.split(",")[0]} — ${fmtGrade10FromPct(s.currentPerformancePct)}`,
        ...
        .join(
          "<br>",
        )}${medios.length > 5 ? `<br>… y ${medios.length - 5} más` : ""}`;
    }

    // ── Riesgo general
    if (c.includes("riesgo") || c.includes("risk")) {
      const ok = n - altos.length - medios.length - zeros.length;
      return `Riesgo en <strong>${courseName}</strong>:<br>Alto: ${altos.length} · Medio: ${medios.length} · OK: ${ok} · Sin nota: ${zeros.length}.<br>${
        altos.length > 0
          ? `Prioridad: ${altos
              .slice(0, 3)
              .map((s) => s.displayName.split(",")[0])
    ...
    if (c.includes("alerta")) {
      const crit = altos.filter(
        (s) => s.currentPerformancePct != null && s.currentPerformancePct < 50,
      );
      return `Sin nota: <strong>${zeros.length}</strong> · Nota menor a 5: <strong>${crit.length}</strong>.<br>${
        crit.length
          ? crit
              .slice(0, 3)
              .map(
    ...
    if (
      c.includes("top") ||
      c.includes("mejor") ||
      c.includes("destacado") ||
      (c.includes("cuales") && c.includes("top"))
    ) {
      const sorted = [...withGrades]
        .sort((a, b) => b.currentPerformancePct - a.currentPerformancePct)
        .slice(0, 3);
    ...
    if (
      c.includes("sin nota") ||
      c.includes("sin evidencia") ||
      c.includes("ruta 0")
    ) {
      if (!zeros.length) return "Todos los estudiantes tienen nota registrada.";
      return `Sin nota: <strong>${zeros.length}</strong>:<br>${zeros
        .slice(0, 5)
        .map((s) => `‣ ${s.displayName.split(",")[0]}`)
        .join(
          "<br>",
    ...
    if (
      ...
      (c.includes("50") && c.includes("cobertura"))
    ) {
      const avgCov = overview?.courseGradebook?.avgCoveragePct;
      const lowCov = (Array.isArray(studentRows) ? studentRows : []).filter(
        (s) => s.coveragePct != null && s.coveragePct < 50,
      );
    ...
    if (
      c.includes("promedio") ||
      c.includes("nota promedio") ||
      c.includes("cual es la nota")
    ) {
    ...
    if (
      c.includes("ruta") ||
      c.includes("intervencion") ||
      c.includes("prescripcion") ||
      c.includes("plan activo")
    ) {
      const routeCounts = {
        route_coverage: 0,
        route_high_risk: 0,
  ...
  function sendMsg(text, fromVoice = false) {
    ...
    setTimeout(
      () => {
        ...
        setMsgs((prev) => [
          ...prev,
          { id: bid, role: "bot", fromVoice: false, text: resp },
        ]);
        setAiStatus("idle");
  ...
  const SM = {
    idle: {
      icon: "",
      label: "Listo para instrucciones",
      sub: "Escribe o usa el micrófono",
      color: "var(--muted)",
    },
    listening: {
      icon: "",
      label: "Escuchando…",
      sub: liveText || "Habla en español",
      color: "var(--critical)",
    },
    thinking: {
      icon: "",
      label: "Analizando datos…",
      sub: "Procesando tu consulta",
      color: "var(--brand)",
    },
    speaking: {
      icon: "",
      label: "Respondiendo en voz…",
      sub: "Haz clic en ⏹ para detener",
      color: "var(--ok)",
    },
  };
  ...
  return (
    <div className="ai-panel">
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div
            style={{
              ...
              animation:
                aiStatus !== "idle" ? "pulse 1.4s ease infinite" : "none",
            }}
          />
          <div style={{ fontSize: 14, fontWeight: 800, color: "var(--text)" }}>
            Asistente IA Académica
          </div>
          <span
            className="tag"
            style={{
      ...
      <div className={`ai-status-outer ${aiStatus !== "idle" ? aiStatus : ""}`}>
        <div className="ai-status-icon" style={{ fontSize: 18 }}>
          {sm.icon}
        </div>
        <div style={{ flex: 1 }}>
          <div
          ...
          <div className="ai-wave">
            {[1, 2, 3, 4, 5].map((n) => (
              <div
                ...
                style={{
                  background:
                    aiStatus === "listening" ? "var(--critical)" : "var(--ok)",
                }}
              />
            ))}
          </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
        ...
        >
          ...
          <button
            onClick={() => {
              const shuffled = [...SUGGESTION_BANK]
        ...
        {msgs.map((m) => (
          <div key={m.id} className={`ai-bubble-wrap ${m.role}`}>
            <div className="ai-meta">
              ...
              {m.fromVoice && <span className="ai-voice-badge"> voz</span>}
            </div>
            <div
      ...
      >
        ...
        >
          ...
          <span
            style={{
              fontSize: 11,
              fontWeight: 700,
              color: voiceOut ? "var(--ok)" : "var(--muted)",
            }}
          >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <button
            className={`ai-stop-btn${aiStatus === "speaking" ? " visible" : ""}`}
        ...
        {[
          {
            ...
            desc: "Presiona el micrófono y habla en español. La transcripción se procesa automáticamente.",
          },
          {
            icon: "",
            color: "var(--ok)",
            title: "Salida de Voz",
            desc: "Activa la voz y el asistente leerá cada respuesta. Usa ' Escuchar' en mensajes anteriores.",
          },
          {
            icon: "",
            color: "var(--watch)",
            title: "Datos Reales",
            desc: "Todas las respuestas usan los datos del curso en tiempo real — notas, cobertura, riesgo y RAs.",
          },
        ].map((g) => (
          <div
            key={g.title}
...
function parseVoiceCommand(rawText) {
  ...
  if (!text)
    return { type: "unknown", message: "No se reconoció ningún comando." };
  ...
  if (
    ...
    return {
      ...
      message: "Mostrando resultados de aprendizaje.",
    };
  }
  if (
    includesAny(text, [
    ...
    return {
      type: "highest_risk_student",
      message: "Buscando el estudiante con mayor riesgo académico.",
    };
  }
  if (
    includesAny(text, [
    ...
    return {
      type: "lowest_result_student",
      message: "Buscando el estudiante con menor desempeño.",
    };
  }
  if (
    includesAny(text, [
      ...
      "solo riesgo",
      "muestrame los de riesgo",
      "filtrar riesgo",
    ])
  ) {
    return {
      type: "filter_students_risk",
      message: "Filtrando estudiantes en riesgo.",
    };
  }
  if (
    includesAny(text, ["evidencias", "abre evidencias", "mostrar evidencias"])
  ) {
    return {
      type: "open_drawer_tab",
      tab: "evidencias",
      message: "Abriendo evidencias.",
    };
  }
  ...
  if (includesAny(text, ["intervencion", "prescripcion"])) {
    return {
      type: "open_drawer_tab",
      tab: "prescripcion",
      message: "Abriendo intervención personalizada.",
    };
  }
  if (includesAny(text, ["calidad", "flags", "calidad del modelo"])) {
    return {
      type: "open_drawer_tab",
      tab: "calidad",
      message: "Abriendo calidad del modelo.",
    };
  }
  if (includesAny(text, ["resumen", "volver al resumen"])) {
    return {
      type: "open_drawer_tab",
      tab: "resumen",
      message: "Abriendo resumen del estudiante.",
    };
  }
  if (includesAny(text, ["aprobados", "aprobado", "pasando"])) {
    return {
      type: "filter_approved",
      message: "Mostrando estudiantes aprobados (≥7.0).",
    };
  }

  const buscarMatch = text.match(
    /(?:busca|buscar|abrir|abre|mostrar|muestrame)\s+a?\s*([a-zà-ü\s]+)$/i,
  );
  if (buscarMatch?.[1] && buscarMatch[1].trim().length >= 3) {
    return {
      type: "find_student_by_name",
      name: buscarMatch[1].trim(),
      message: `Buscando a ${buscarMatch[1].trim()}.`,
    };
  }
  if (includesAny(text, ["estudiantes", "lista de estudiantes"])) {
    return {
      type: "navigate_section",
      section: "students",
      message: "Mostrando listado de estudiantes.",
    };
  }
  if (text.length >= 3) {
    return {
      type: "text_search",
      text: rawText,
      message: `Buscando: ${rawText}`,
    };
  }
  return {
    type: "unknown",
    message:
      "No se entendió el comando. Prueba: 'estudiante con resultado más bajo' o 'resultados de aprendizaje'.",
  };
}
...
function findLowestResultStudent(rows) {
  const valid = (Array.isArray(rows) ? rows : []).filter(
    (s) =>
      !s?.isLoading &&
      s?.currentPerformancePct != null &&
      !Number.isNaN(Number(s.currentPerformancePct)),
  );
  ...
  return valid
    ...
    .sort(
      (a, b) =>
        Number(a.currentPerformancePct) - Number(b.currentPerformancePct),
    )[0];
}
...
function findHighestRiskStudent(rows) {
  ...
  return valid.slice().sort((a, b) => {
    ...
    if (rd !== 0) return rd;
    return (
      Number(a?.currentPerformancePct ?? 999) -
      Number(b?.currentPerformancePct ?? 999)
    );
  })[0];
}
...
function findStudentByName(rows, name) {
  ...
  return (
    (Array.isArray(rows) ? rows : []).find((s) =>
      normalizeVoiceText(s?.displayName).includes(q),
    ) || null
  );
}
...
function CourseItem({ course, isActive, isCurrent, onSelect, accent }) {
  const startYear = course.startDate
    ? new Date(course.startDate).getFullYear()
    : null;
  const endYear = course.endDate
...
function StudentCard({ s, onOpen, weakestMacro }) {
  ...
  return (
    ...
    >
      ...
      >
        <CircularRing
          pct={s.currentPerformancePct ?? 0}
          size={56}
          stroke={5}
          color={gradeColor}
          label={fmtGrade10FromPct(s.currentPerformancePct)}
          fontSize={11}
        />
        <CircularRing
          pct={s.coveragePct ?? 0}
...
function GradeDistributionCard({ studentRows, thresholds }) {
  ...
  const bajos = rows.filter(
    (s) => computeRiskFromPct(s.currentPerformancePct) === "bajo",
  );
  const medios = rows.filter(
    (s) => computeRiskFromPct(s.currentPerformancePct) === "medio",
  );
  const altos = rows.filter(
    (s) => computeRiskFromPct(s.currentPerformancePct) === "alto",
  );
    ...
    <Card
      title={
        <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
          ...
          <InfoTooltip text="Histograma de notas de los estudiantes en rangos de 1 punto. Los colores reflejan el estado: rojo=crítico (<5), amarillo=seguimiento (5-7), verde=óptimo (≥7). Excluye columnas 'Corte' para evitar doble conteo." />
        </span>
      }
      accent="brand"
    >
          ...
          >
            ...
            >
              <div
                style={{
                  height: "100%",
                  width: `${b.count ? Math.max(8, Math.round((b.count / maxBand) * 100)) : 0}%`,
                  background: b.color,
                  borderRadius: 5,
            ...
            <span
              style={{
                ...
                fontWeight: 900,
                fontFamily: "var(--font-mono)",
                color: b.count ? b.color : "var(--muted)",
                width: 20,
                textAlign: "right",
                flexShrink: 0,
              }}
...
const ROUTE_DEFS = {
  route_coverage: {
    ...
    icon: "",
    description:
      "El estudiante tiene muy poca cobertura de evaluación. La prioridad es identificar evidencias sin calificar y activarlas antes de que el semestre avance.",
  ...
  route_high_risk: {
    ...
    icon: "",
    description:
      "El estudiante está en riesgo alto. Su nota actual está por debajo del umbral crítico. Se requiere intervención inmediata con plan estructurado de corto plazo.",
  ...
  route_watch: {
    ...
    icon: "",
    description:
      "El estudiante está en riesgo medio. Su desempeño es insuficiente en algún resultado de aprendizaje específico. El ajuste debe ser puntual y enfocado.",
  ...
  route_ok: {
    ...
    icon: "",
    description:
      "El estudiante tiene buen desempeño. La gestión aquí es de sostenimiento y motivación para que mantenga el ritmo hasta el cierre del semestre.",
...
function RoutesView({
  ...
  const totalAssigned = Object.values(byRoute).reduce(
    (a, arr) => a + arr.length,
    0,
  );
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div>
        ...
        >
          {totalAssigned} estudiantes asignados ·{" "}
          {Object.values(byRoute).filter((arr) => arr.length > 0).length} rutas
          activas
        </div>
      </div>
            ...
            <div
              ...
              onMouseEnter={(e) => {
                if (!isSelected) {
                  ...
                  e.currentTarget.style.background = route.bg;
                }
              }}
              onMouseLeave={(e) => {
                if (!isSelected) {
    ...
    <header className="app-topbar">
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            {showImpersonateMenu && (
              ...
              >
                ...
                >
                  ...
                  {(Array.isArray(studentRows) ? studentRows : []).length ===
                    0 && (
                    <div
                      style={{
                        padding: "16px 14px",
        ...
        <button
          ...
          aria-label={
            compact ? "Desactivar modo compacto" : "Activar modo compacto"
          }
          style={
            compact
        ...
        >
          ...
          >
            {authUser?.user_name
              ? authUser.user_name.trim().charAt(0).toUpperCase()
              : "D"}
          </div>
          {authUser?.user_name && (
            <span
      ...
      <div className="ai-fab">
        {open && (
          <div
            ...
            style={{ width: 250, maxHeight: "none" }}
          >
            <div
              className="ai-fab-panel-header"
        ...
        >
          <span
            style={{ color: "#fff", fontSize: open ? 20 : 22, fontWeight: 900 }}
          >
            {open ? "" : "?"}
          </span>
        </button>
...
export default function TeacherDashboard() {
  ...
  const {
    initialOrgUnitId: ctxInitialOrgUnitId,
    isDualRole,
    isSuperAdmin,
  } = useAuth();
      ...
      try {
        ...
        if (_hash.startsWith("#gemelo:")) {
          ...
          if (parts.length >= 2) {
            _sid = parts[1] || null;
            _hashOu =
              parts[2] && Number(parts[2]) > 0 ? Number(parts[2]) : null;
            const _fl = parts[3];
        ...
        if (data.authenticated) {
          ...
          const isFirstLogin =
            sessionStorage.getItem("gemelo_first_login") === "1";
          const alreadyOnboarded =
            localStorage.getItem("gemelo_onboarded") === "1";
  ...
  const [csvColumns, setCsvColumns] = useState(() =>
    STUDENT_CSV_COLUMNS.map((c) => c.key),
  );
  ...
  const searchCourses = React.useCallback(async (term) => {
    ...
    try {
      ...
      const enrolledItems =
        enrolledData.status === "fulfilled"
          ? Array.isArray(enrolledData.value?.items)
            ? enrolledData.value.items
            : []
          : [];
      const myItems =
        myData.status === "fulfilled"
          ? Array.isArray(myData.value?.items)
            ? myData.value.items
            : []
          : [];
  ...
  const executeVoiceCommand = React.useCallback(
    (rawText) => {
      ...
      switch (command.type) {
        ...
        case "lowest_result_student": {
          ...
          if (student) {
            ...
            setVoiceFeedback(
              `Abriendo a ${student.displayName} — nota más baja: ${fmtGrade10FromPct(student.currentPerformancePct)}.`,
            );
          } else {
            setVoiceFeedback(
              "No encontré estudiantes con calificación disponible.",
            );
          }
          return;
        }
        case "highest_risk_student": {
          ...
          if (student) {
            ...
            setDrawerTab("resumen");
            setVoiceFeedback(
              `Abriendo a ${student.displayName} — estudiante priorizado por riesgo académico.`,
            );
          } else {
        ...
        case "find_student_by_name": {
          ...
          if (student) {
            ...
            setActiveSection("students");
            setVoiceFeedback(
              `No encontré coincidencia exacta. Buscando: "${command.name}".`,
            );
          }
          return;
        }
        case "open_drawer_tab": {
          ...
          if (!selectedStudent) {
            ...
            if (fallback) {
              ...
              setVoiceFeedback(
                `Abriendo ${command.tab} para ${fallback.displayName}.`,
              );
            } else {
              setVoiceFeedback(
                "No hay estudiante seleccionado. Abre uno primero.",
              );
            }
          } else {
            setDrawerTab(command.tab);
            setVoiceFeedback(
              `Abriendo ${command.tab} para ${selectedStudent.displayName}.`,
            );
          }
          return;
        }
    ...
    (async () => {
      try {
        const [ovRes, stRes, raRes, loRes] = await Promise.allSettled([
          ...
          apiGet(`/gemelo/course/${orgUnitId}/students?include=summary`, {
            signal: controller.signal,
          }),
          apiGet(`/gemelo/course/${orgUnitId}/ra/dashboard`, {
            signal: controller.signal,
          }),
          apiGet(`/gemelo/course/${orgUnitId}/learning-outcomes`, {
            signal: controller.signal,
          }),
        ]);
        ...
        if (loRes.status === "fulfilled") {
          ...
          const sets = Array.isArray(payload?.outcomeSets)
            ? payload.outcomeSets
            : [];
          const map = {};
        ...
        const hasInlineSummary =
          studentItems.length > 0 && (_hasMeaningfulData || _hasStructure);
        ...
        if (hasInlineSummary) {
          const details = studentItems.map((s) => {
            ...
            const gradedItemsCount =
              sum?.gradedItemsCount ?? sum?.coverageGradedCount ?? null;
            const totalItemsCount =
              sum?.totalItemsCount ?? sum?.coverageTotalCount ?? null;
            const coverageCountText =
              sum?.coverageCountText ??
              (gradedItemsCount != null && totalItemsCount != null
                ? `${gradedItemsCount}/${totalItemsCount}`
                : null);
            ...
            const row = {
              ...
              hasPrescription: Boolean(
                sum?.hasPrescription ?? s?.hasPrescription ?? false,
              ),
              ...
              pendingSubmittedCount:
                sum?.pendingUngradedCount ?? sum?.pendingSubmittedCount ?? 0,
              pendingSubmittedWeightPct:
                sum?.pendingUngradedWeightPct ??
                sum?.pendingSubmittedWeightPct ??
                0,
              overdueCount: sum?.overdueUnscoredCount ?? sum?.overdueCount ?? 0,
              overdueWeightPct:
                sum?.overdueUnscoredWeightPct ?? sum?.overdueWeightPct ?? 0,
              notSubmittedCount:
                sum?.overdueUnscoredCount ?? sum?.notSubmittedCount ?? 0,
              notSubmittedWeightPct:
                sum?.overdueUnscoredWeightPct ??
                sum?.notSubmittedWeightPct ??
                0,
            };
            ...
            setStudentRows((prev) =>
              prev.map((row) => {
                ...
                const merged = {
                  ...
                  notSubmittedWeightPct: Number(
                    ar.overdueUnscoredWeightPct ??
                      ar.notSubmittedWeightPct ??
                      0,
                  ),
                  overdueWeightPct: Number(
                    ar.overdueUnscoredWeightPct ??
                      ar.notSubmittedWeightPct ??
                      0,
                  ),
                  pendingSubmittedWeightPct: Number(
                    ar.pendingUngradedWeightPct ??
                      ar.pendingSubmittedWeightPct ??
                      0,
                  ),
                  // mostCriticalMacro now included from backend studentsAtRisk
                  mostCriticalMacro:
                    ar.mostCriticalMacro ?? row.mostCriticalMacro ?? null,
                };
        ...
        setStudentRows((prev) =>
          prev.map((row) => {
            ...
            const merged = {
              ...
              risk: computeRiskFromPct(perf),
              notSubmittedWeightPct: Number(
                ar?.overdueUnscoredWeightPct ?? ar?.notSubmittedWeightPct ?? 0,
              ),
              overdueWeightPct: Number(
                ar?.overdueUnscoredWeightPct ?? ar?.notSubmittedWeightPct ?? 0,
              ),
    ...
    (async () => {
      try {
        ...
        if (contentRes.status === "fulfilled") {
          setContentRoot(
            Array.isArray(contentRes.value) ? contentRes.value : [],
          );
        } else {
    ...
    (async () => {
      try {
        const g = await apiGet(
          `/gemelo/course/${orgUnitId}/student/${selectedStudent.userId}`,
          {
            signal: controller.signal,
          },
        );
        ...
        const hasEvidences =
          Array.isArray(g?.gradebook?.evidences) &&
          g.gradebook.evidences.length > 0;
        ...
        const rowMatch = studentRows.find(
          (r) => r.userId === selectedStudent.userId,
        );
  ...
  const resetThresholds = () => {
    setThresholdsOverride(null);
    try {
      localStorage.removeItem(`gemelo_thresholds_${orgUnitId}`);
    } catch {
      /* ignore */
    }
  };
  ...
  const weakestMacro = useMemo(() => {
    if (!Array.isArray(learningOutcomesData) || !learningOutcomesData.length)
      return null;
  ...
  const assignmentRiskData = useMemo(() => {
    ...
    const resolveName = (userId, fallbackName) => {
      ...
      if (fallbackName && String(fallbackName) !== String(userId))
        return fallbackName;
      return `Estudiante ${userId}`;
    };

    // Fuente 1: overview.studentsAtRisk (backend)
    const backendRisk = Array.isArray(overview?.studentsAtRisk)
    ...
    if (backendRisk.length > 0) {
      candidates = backendRisk.map((s) =>
        toItem(
          ...
          Number(
            s.pendingUngradedWeightPct ?? s.pendingSubmittedWeightPct ?? 0,
          ),
          s.coveragePct,
        ),
      );
    ...
    const filtered = candidates.filter((s) => {
      ...
      if (s.risk === "pending")
        return (
          s.coveragePct < 60 ||
          s.notSubmittedWeightPct > 0 ||
          s.pendingSubmittedWeightPct > 0
        );
    ...
    filtered.sort((a, b) => {
      ...
      if (ro !== 0) return ro;
      return (
        (a.currentPerformancePct ?? 999) - (b.currentPerformancePct ?? 999)
      );
    });
  ...
  const avgPerfPct =
    overview?.courseGradebook?.avgCurrentPerformancePct ?? null;
  const avgCov = overview?.courseGradebook?.avgCoveragePct ?? null;
  const covDone =
    avgCov == null ? 0 : Math.max(0, Math.min(100, Number(avgCov)));
  ...
  const avgOverdueUnscoredPct = useMemo(() => {
    ...
    const atRisk = Array.isArray(overview?.studentsAtRisk)
      ? overview.studentsAtRisk
      : [];
    if (atRisk.length > 0) {
  ...
  const covPending = Math.max(
    ...
    Math.min(
      100,
      100 - covDone - avgPendingUngradedPct - avgOverdueUnscoredPct,
    ),
  );
  ...
  const atRiskPct =
    totalStudents > 0 ? (atRiskCount / totalStudents) * 100 : null;
  ...
  const courseStatus = useMemo(() => {
    ...
    const a = loaded.filter(
      (s) => computeRiskFromPct(s.currentPerformancePct) === "alto",
    ).length;
    const m = loaded.filter(
      (s) => computeRiskFromPct(s.currentPerformancePct) === "medio",
    ).length;
    const b = loaded.filter(
      (s) => computeRiskFromPct(s.currentPerformancePct) === "bajo",
    ).length;
  ...
  const filteredStudents = useMemo(() => {
    ...
    if (advancedQuery.mode === "students-at-risk") {
      return list.filter((s) =>
        ["alto", "medio"].includes(computeRiskFromPct(s.currentPerformancePct)),
      );
    }
    if (advancedQuery.mode === "approved") {
      return list.filter(
        (s) => s.currentPerformancePct != null && s.currentPerformancePct >= 70,
      );
    }
    ...
    if (quickFilter === "risk_high") {
      ...
      list = list.filter(
        (s) => (s.notSubmittedWeightPct ?? s.overdueWeightPct ?? 0) > 0,
      );
  ...
  const contentKpis = useMemo(() => {
    ...
    const progressRatio =
      minExpected > 0 ? clamp(createdCount / minExpected, 0, 2) : null;
  ...
  const sortedStudents = useMemo(() => {
    ...
    const getVal = (s) => {
      switch (sortKey) {
        ...
        case "grade10":
          return s.currentPerformancePct == null
            ? -1
            : Number(s.currentPerformancePct) / 10;
        case "coverage":
  ...
  const drawerMacro = (
    studentDetail?.macroUnits ||
    studentDetail?.macro?.units ||
    []
  ).map((u) => ({
    code: u.code,
    pct: Number(u.pct || 0),
  ...
  const drawerPrescription = Array.isArray(studentDetail?.prescription)
    ? studentDetail.prescription
    : [];
  const drawerProjection = studentDetail?.projection || null;
  const drawerGradebook = studentDetail?.gradebook || {};
  const drawerEvidences = Array.isArray(drawerGradebook?.evidences)
    ? drawerGradebook.evidences
    : [];
  const drawerGradeCategories = Array.isArray(drawerGradebook?.gradeCategories)
    ? drawerGradebook.gradeCategories
    : [];
  const drawerPendingItems = Array.isArray(drawerGradebook?.pendingItems)
    ? drawerGradebook.pendingItems
    : [];
  const drawerMissingValues = Array.isArray(drawerGradebook?.missingValues)
    ? drawerGradebook.missingValues
    : [];
  const drawerQcFlags = Array.isArray(studentDetail?.qualityFlags)
    ? studentDetail.qualityFlags
    : [];
  const drawerPendingUngradedPct = Number(
    drawerSummary?.pendingUngradedWeightPct ?? 0,
  );
  const drawerOverdueUnscoredPct = Number(
    drawerSummary?.overdueUnscoredWeightPct ?? 0,
  );
  const covGraded =
    Number(
      drawerSummary?.gradedItemsCount ?? drawerGradebook?.gradedItemsCount ?? 0,
    ) || 0;
  const covTotal =
    Number(
      drawerSummary?.totalItemsCount ?? drawerGradebook?.totalItemsCount ?? 0,
    ) || 0;
  const covText =
  ...
  const drawerTabs = [
    { id: "resumen", label: "Resumen", icon: "" },
    {
      ...
      label: "Evidencias",
      icon: "",
      count: drawerEvidences.length || undefined,
    },
    {
      id: "unidades",
      label: "Unidades",
      icon: "",
      count: drawerUnits.length || undefined,
    },
    { id: "notas", label: "Mis notas", icon: "" },
    {
      ...
      label: "Historial",
      icon: "",
      count: studentChatHook.entries.length || undefined,
    },
      ...
      ? [
          {
            ...
            label: "Intervención",
            icon: "",
            count: drawerPrescription.length,
          },
        ]
      : []),
    ...(drawerQcFlags.filter((f) => f?.type && f.type !== "role_not_enabled")
      .length
  ...
  const { snapshots: courseSnapshots } = useCourseSnapshots(
    orgUnitId,
    snapshotMetrics,
  );

  // Helper to select a student by userId from SmartAlerts chips
  const selectStudentById = React.useCallback(
    (uid) => {
  ...
  const paletteCommands = useMemo(() => {
    ...
    cmds.push({
      ...
      group: "Acciones",
      icon: "",
      label: "Cambiar de curso",
      action: () => handleOpenCoursePanel(),
    });
    cmds.push({
      id: "act_refresh",
      group: "Acciones",
    ...
    cmds.push({
      ...
      group: "Acciones",
      icon: compact ? "◱" : "◰",
      label: compact ? "Desactivar modo compacto" : "Activar modo compacto",
    ...
    cmds.push({
      ...
      group: "Acciones",
      icon: "",
      label: groupByRisk ? "Desagrupar tabla" : "Agrupar tabla por riesgo",
      action: () => setGroupByRisk((v) => !v),
    });
    // Filters
    cmds.push({
      id: "fil_high",
    ...
    cmds.push({
      ...
      group: "Filtros",
      icon: "",
      label: "Solo riesgo medio",
      action: () => {
        setQuickFilter("risk_medium");
        setActiveSection("students");
      },
    });
    cmds.push({
    ...
    cmds.push({
      ...
      group: "Filtros",
      icon: "⏳",
      label: "Con entregas pendientes por calificar",
      action: () => {
        setQuickFilter("pending_grade");
        setActiveSection("students");
      },
    });
    cmds.push({
  ...
  useKeyboardShortcuts(
    [
      ...
      {
        keys: "2",
        handler: () => setActiveTab("routes"),
        description: "Rutas",
      },
      {
        keys: "3",
        handler: () => setActiveTab("assistant"),
        description: "Asistente",
      },
      { keys: "r", handler: handleRefresh, description: "Refrescar" },
      {
        keys: "c",
        handler: handleOpenCoursePanel,
        description: "Cambiar curso",
      },
      ...
      >
        ...
        >
          ...
          >
            ...
            <div>
              <div
                style={{ fontSize: 15, fontWeight: 900, color: "var(--text)" }}
              >
                G.D
              </div>
              <div
              ...
              >
                Hola, {authUser?.user_name?.split(" ")[0] || "docente"} —
                selecciona tu curso
              </div>
            </div>
          </div>
          ...
          {loadingCourses ? (
            ...
            (() => {
              const instructorCourses = courseList.filter(
                (c) => !isStudentRole(c.roleName),
              );
              const studentCourses = courseList.filter((c) =>
                isStudentRole(c.roleName),
              );
                  ...
                  {instructorCourses.length > 0 && (
                    <div>
                      ...
                      >
                        ...
                        >
                          Como Profesor ({instructorCourses.length})
                        </span>
                      </div>
                      <div
                        style={{
                  ...
                  {studentCourses.length > 0 && (
                    <div>
                      ...
                      >
                        ...
                        >
                          Como Estudiante ({studentCourses.length})
                        </span>
                      </div>
                      <div
                        style={{
  ...
  if (err) {
    ...
    const isNoAccess =
      err.includes("401") ||
      err.includes("403") ||
      err.includes("autenticado") ||
      err.includes("No tiene acceso");
    const isNotFound =
      err.includes("404") ||
      err.includes("not found") ||
      err.includes("no encontrado");
    ...
    return (
      ...
      >
        <div
          style={{
            background: "var(--card)",
            border: `1.5px solid ${isNoAccess ? "var(--watch)" : isNotFound ? "var(--muted)" : "var(--critical)"}`,
            borderRadius: 18,
            padding: "40px 44px",
            maxWidth: 460,
            width: "100%",
  ...
  if (!overview)
    return <CesaLoader subtitle="Inicializando información del curso..." />;
  ...
  return (
    ...
    >
      ...
      <AppSidebar
        ...
        currentCourseName={
          courseInfo?.Name || (orgUnitId ? `Curso ${orgUnitId}` : null)
        }
                ...
                >
                  <div>
                    ...
                    >
                      ...
                      {avgPerfPct != null && avgPerfPct > 0
                        ? ` · Promedio ${fmtGrade10FromPct(avgPerfPct)}/10`
                        : ""}
                      {courseInfo?.StartDate
                        ? ` · ${new Date(courseInfo.StartDate).getFullYear()}`
                        : ""}
                    </div>
                  </div>
                  <div
                ...
                <Card
                  title={
                    ...
                    >
                      {" "}
                      Resumen semanal{" "}
                      <InfoTooltip text="Resumen narrativo en lenguaje natural del estado del curso. Se genera automáticamente a partir de los datos actuales. Puedes escucharlo con TTS." />
                    </span>
                  }
                  accent="brand"
                >
              ...
              <div
                ...
                style={{
                  ...
                  gridTemplateColumns: isMobile
                    ...
                    : isNarrow
                      ? "1fr 1fr"
                      : "minmax(260px,2fr) minmax(180px,1fr) minmax(200px,1.4fr) minmax(180px,1.2fr)",
                  gap: 16,
                  marginBottom: 16,
                  alignItems: "start",
                }}
              >
                <div ref={overviewRef}>
                  <Card
                    ...
                    right={<StatusBadge status={courseStatus} />}
                    accent="brand"
                  >
                    <div
                      style={{
                    ...
                    >
                      <Stat
                        ...
                        value={
                          avgPerfPct == null || Number(avgPerfPct) === 0
                            ? "—"
                            : fmtGrade10FromPct(avgPerfPct)
                        }
                    ...
                    <Stat
                      ...
                      sub={
                        totalStudents
                          ? `${atRiskCount} de ${totalStudents} estudiantes`
                          : "—"
                      }
                    />
                    ...
                    >
                      ...
                      <div style={{ marginLeft: "auto" }}>
                        <span
                          className="badge"
                          style={{
                            background: contentRhythmMeta.bg,
                        ...
                        >
                          <span
                            className="pulse-dot"
                            style={{
                              background: contentRhythmMeta.color,
                              width: 6,
                              height: 6,
                            }}
                          />
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          {contentKpis?.minExpected == null
                            ? "—"
                            : contentKpis.minExpected}
                        </div>
                        <div
                          style={{
                            fontSize: 11,
                ...
                >
                  <Card
                    title={
                      ...
                      >
                        Riesgo académico{" "}
                        <InfoTooltip text="Distribución de los estudiantes según su nota actual: Alto (<5.0), Medio (5.0–7.0), Bajo (≥7.0). Calculado solo con notas reales del gradebook, excluye columnas 'Corte'." />
                      </span>
                    }
                    accent="pending"
                  >
                ...
                <div ref={priorityRef}>
                  <Card
                    title={
                      ...
                      >
                        Estudiantes prioritarios{" "}
                        <InfoTooltip text="Estudiantes que requieren tu atención inmediata: nota crítica (<5), cobertura baja (<60%), ítems vencidos sin calificar o pendientes de calificación. Ordenados por nivel de riesgo." />
                      </span>
                    }
                    accent="critical"
                    right={
                  ...
                  >
                    ...
                    !assignmentRiskData.length ? (
                      ...
                      >
                        ...
                        >
                          {assignmentRiskData.map((item) => {
                            ...
                            const grade10 =
                              item.currentPerformancePct != null
                                ? (item.currentPerformancePct / 10).toFixed(1)
                                : null;
                            const gradeColor =
                              item.currentPerformancePct != null
                                ? colorForPct(
                                    item.currentPerformancePct,
                                    thresholds,
                                  )
                                : COLORS.pending;
                            const borderColor = hasLowGrade
                              ? "#FECDCA"
                              : hasOverdue
                                ? "#FED7AA"
                                : "var(--border)";
                            const bgColor = hasLowGrade
                              ? "var(--critical-bg)"
                              : hasOverdue
                                ? "var(--watch-bg)"
                                : "var(--card)";
                                  ...
                                  >
                                    ...
                                    {item.pendingSubmittedWeightPct > 0 && (
                                      ...
                                      >
                                        ...
                                        >
                                          {fmtPct(
                                            item.pendingSubmittedWeightPct,
                                          )}
                                        </div>
                                      </div>
                                    )}
                                    {hasOverdue && (
                                      ...
                                      >
                                        ...
                                        >
                                          {fmtPct(item.notSubmittedWeightPct)}
                                        </div>
                                      </div>
                                    )}
                                  </div>
                      ...
                      <div className="empty-state" style={{ minHeight: 160 }}>
                        ...
                        >
                          Todos los estudiantes tienen cobertura ≥ 60% y sin
                          ítems vencidos.
                        </span>
                      </div>
                    )}
                ...
                <div ref={learningOutcomesRef}>
                  <Card
                    title={
                      ...
                      >
                        Prioridad académica{" "}
                        <InfoTooltip text="Resultados de Aprendizaje (RA) del curso ordenados de menor a mayor desempeño. El RA en primera posición es donde tus estudiantes están más débiles — prioriza refuerzo ahí." />
                      </span>
                    }
                    accent="brand"
                  >
                                ...
                                m.coveragePct != null ? (
                                  <div>
                                    ...
                                    >
                                      ...
                                      {m.studentsWithData}/{m.totalStudents}{" "}
                                      est.
                                    </div>
                                  </div>
                                ) : m.studentsWithData === 0 ? (
                      ...
                      {!learningOutcomesData.length && (
                        <div className="empty-state">
                          ...
                          {Number(avgCov ?? 0) > 0 && (
                            ...
                            >
                              Hay evidencias calificadas pero sin rúbricas
                              vinculadas a RA
                            </span>
                          )}
                        </div>
                ...
                <Card
                  title={
                    ...
                    >
                      ...
                      <InfoTooltip text="Evolución de nota promedio, porcentaje en riesgo y cobertura a lo largo de los últimos días. Los datos se capturan automáticamente cada vez que abres el dashboard." />
                    </span>
                  }
                  accent="brand"
                >
              ...
              <div className="fade-up fade-up-3" style={{ marginBottom: 16 }}>
                <Card
                  title={
                    ...
                    >
                      Calendario de entregas{" "}
                      <InfoTooltip text="Próximas entregas del curso con detección de sobrecarga (3+ en el mismo día). Heatmap semanal al final. Toma los datos directamente del gradebook del curso." />
                    </span>
                  }
                >
                  <DueDateCalendar
                  ...
                  right={
                    ...
                    >
                        ...
                        <div style={{ display: "flex", gap: 4 }}>
                          <button
                            ...
                            onClick={() => {
                              ...
                              if (advancedQuery?.mode)
                                activeFilters.push(advancedQuery.mode);
                      ...
                      <button
                        className="btn"
                        onClick={() =>
                          exportCourseReport(studentRows, courseInfo, overview)
                        }
                ...
                >
                  ...
                  >
                    {[
                      ...
                      { id: "overdue", label: "Vencidos", icon: "" },
                      {
                        id: "pending_grade",
                        label: "Pendientes calificación",
                        icon: "⏳",
                      },
                    ...
                    <button
                      ...
                      style={{
                        fontSize: 11,
                        marginLeft: "auto",
                        borderColor: groupByRisk ? "var(--brand)" : undefined,
                        color: groupByRisk ? "var(--brand)" : undefined,
                      }}
                    >
                  ...
                  {selectedStudentIds.size > 0 && (
                    ...
                    >
                      ...
                      >
                        {selectedStudentIds.size} estudiante
                        {selectedStudentIds.size !== 1 ? "s" : ""} seleccionado
                      ...
                      >
                        <button
                          ...
                          onClick={() => {
                            ...
                            const selected = studentRows.filter((s) =>
                              selectedStudentIds.has(s.userId),
                            );
                            ...
                            const subject = encodeURIComponent(
                              "Sobre el curso: " + (courseInfo?.Name || ""),
                            );
                            ...
                            const rows = [
                              ...
                              ...selected.map((s) =>
                                [
                                  ...
                                  s.email || "",
                                  s.currentPerformancePct != null
                                    ? (s.currentPerformancePct / 10).toFixed(1)
                                    : "",
                                  s.coveragePct != null
                  ...
                  {voiceSupported && !voiceFeedback && (
                    <div className="voice-hint" style={{ marginBottom: 10 }}>
                      <span></span>
                      Prueba: <em>"resultado más bajo"</em> ·{" "}
                      <em>"resultados de aprendizaje"</em> ·{" "}
                      <em>"estudiantes en riesgo"</em>
                    </div>
                  )}
                        ...
                        <thead>
                          ...
                          >
                            ...
                            <SortTh
                              label="Cobertura"
                              {...makeSort("coverage")}
                              title="% del curso con evidencias calificadas"
                            />
                            <th />
                          </tr>
                        </thead>
                            ...
                            const renderStudentRow = (s) => (
                              <tr
                                ...
                                style={{
                                  borderBottom: "1px solid var(--border)",
                                  cursor: "pointer",
                                  background: selectedStudentIds.has(s.userId)
                                    ? "var(--brand-light)"
                                    : undefined,
                                }}
                              >
                                <td
                                  style={{ padding: "10px 10px", width: 28 }}
                                  onClick={(e) => e.stopPropagation()}
                                >
                                  <input
                                    type="checkbox"
                                    ...
                                    onChange={() =>
                                      toggleStudentSelection(s.userId)
                                    }
                                  />
                                </td>
                                <td
                                  ...
                                  >
                                    ...
                                    <div style={{ flex: 1, minWidth: 0 }}>
                                      ...
                                      >
                                        ...
                                        {s.hasPrescription && (
                                          <span
                                            title="Tiene prescripción activa"
                                            style={{ fontSize: 14 }}
                                          ></span>
                                        )}
                                      </div>
                                ...
                                <td style={{ padding: "10px 10px" }}>
                                  <StatusBadge
                                    status={s.isLoading ? "cargando" : s.risk}
                                  />
                                </td>
                                <td
                                  style={{
                                    padding: "10px 10px",
                                    maxWidth: compactRouteCol ? 200 : 320,
                                    minWidth: 160,
                                  }}
                                >
                                  {s.route ? (
                                    ...
                                    {(() => {
                                      ...
                                      return (
                                        <div
                                          title={
                                            isFallback
                                              ? "RA del curso (sin datos individuales)"
                                              : undefined
                                          }
                                        >
                                          <div
                                  ...
                                  >
                                    ...
                                    >
                                      ...
                                      >
                                        ...
                                        >
                                          {groups[gm.key].length}
                                        </span>
                                      </div>
                                    </td>
                                  </tr>
                                  {!isCollapsed &&
                                    groups[gm.key].map(renderStudentRow)}
                                </React.Fragment>
                              );
                            });
                          })()}
      ...
      <Drawer
        ...
        title={
          selectedStudent ? `${selectedStudent.displayName}` : "Estudiante"
        }
        ...
        {studentLoading ? (
          ...
          >
            <div className="cesa-water-text" style={{ fontSize: 36 }}>
              <span
                className="cesa-water-text__outline"
                style={{ fontSize: 36 }}
              >
                CESA
              </span>
              <span
                className="cesa-water-text__fill"
                aria-hidden="true"
                style={{ fontSize: 36 }}
              >
                CESA
          ...
          >
            <div style={{ color: "var(--muted)" }}>
              No hay datos consolidados para este estudiante.
            </div>
          </Card>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            >
              ...
              >
                <CircularRing
                  ...
                  stroke={8}
                  color={colorForPct(
                    drawerSummary?.currentPerformancePct,
                    thresholds,
                  )}
                  label={fmtGrade10FromPct(
                    drawerSummary?.currentPerformancePct,
                  )}
                  sublabel="/10"
                  fontSize={20}
              ...
              >
                ...
                >
                  ...
                  <StatusBadge
                    status={
                      drawerSummary?.risk || selectedStudent?.risk || "pending"
                    }
                  />
                </div>
                {drawerPendingUngradedPct > 0 && (
                  ...
                  >
                    ...
                    >
                      {fmtPct(drawerPendingUngradedPct)}
                    </span>
                  </div>
                )}
                {drawerOverdueUnscoredPct > 0 && (
                  ...
                  >
                    ...
                    >
                      {fmtPct(drawerOverdueUnscoredPct)}
                    </span>
                  </div>
                )}
                {drawerPendingUngradedPct === 0 &&
            ...
            >
              {drawerTabs.map((tab) => (
                <button
                  key={tab.id}
                  className={`chip ${drawerTab === tab.id ? "active" : ""}`}
              ...
              >
                  ...
                  <Card title="Cobertura de evaluación">
                    <CoverageBars
                      ...
                      openPct={Math.max(
                        0,
                        100 -
                          (drawerSummary?.coveragePct ?? 0) -
                          drawerPendingUngradedPct -
                          drawerOverdueUnscoredPct,
                      )}
                ...
                {drawerMacro.length > 0 ? (
                  <Card title="Resultados de aprendizaje del estudiante">
                    ...
                    >
                      {drawerMacro.map((item) => {
                        ...
                        const isWatch =
                          !isCrit && item.pct < (thresholds?.watch ?? 70);
                    ...
                    {drawerMacro.some((item) => {
                      const ri = learningOutcomesData.find(
                        (r) => r.code === item.code,
                      );
                    ...
                    >
                      ...
                      >
                        ...
                        />{" "}
                        Crítico ({thresholds?.critical ?? 50}%)
                      </span>
                      <span
                        style={{
                          display: "flex",
                      ...
                      >
                        ...
                        />{" "}
                        Observación ({thresholds?.watch ?? 70}%)
                      </span>
                      <button
                  ...
                  <Card title="Resultados de aprendizaje del curso">
                    ...
                    >
                      Sin datos de evaluación por RA para este estudiante aún.
                ...
                {selectedStudent?.route && (
                  <Card
                    title={selectedStudent.route.title}
                    right={
                      <StatusBadge
                        status={computeRiskFromPct(
                          selectedStudent?.currentPerformancePct,
                        )}
                      />
                    }
                  >
                ...
                <PendingItemsBlock
                  pendingItems={drawerPendingItems}
                  missingValues={drawerMissingValues}
                />
              </div>
            )}
            ...
            {drawerTab === "evidencias" && (
              ...
              >
                {drawerEvidences.length > 0 ? (
                  (() => {
                    const drawerCorte = drawerEvidences.filter(
                      (e) => e?.isCorte === true,
                    );
                    const drawerNonCorte = drawerEvidences.filter(
                      (e) => e?.isCorte !== true,
                    );
                    const drawerOverdue = drawerNonCorte.filter(
                      (e) =>
                        e?.status === "overdue_unscored" ||
                        (e?.isOverdue && e?.scorePct == null),
                    );
                    const fmtDue = (iso) => {
                      if (!iso) return "—";
                      try {
                    ...
                    return (
                      <>
                        ...
                        {(() => {
                          const dgroups = buildCorteGroups(
                            drawerEvidences,
                            drawerGradeCategories,
                          );
                          return dgroups.length > 0;
                        })() && (
                          <Card
                            title="Resumen por Cortes"
                            right={<span className="tag">No suman</span>}
                            accent="brand"
                          >
                            <div
                              style={{
                            ...
                            >
                              Ponderados acumulados que Brightspace calcula. Se
                              agrupan por categoría/corte con las evidencias que
                              los componen. <strong>No cuentan</strong> en el
                        ...
                        {drawerOverdue.length > 0 && (
                          <Card
                            title={` Entregas Vencidas (${drawerOverdue.length})`}
                            accent="critical"
                          >
                            <div
                              style={{
                              ...
                              <tbody>
                                {drawerNonCorte.map((e, i) => (
                                  ...
                                  >
                                    ...
                                    >
                                      {e.scorePct != null
                                        ? (Number(e.scorePct) / 10).toFixed(1)
                                        : "—"}
                                    </td>
                                    <td
                                      style={{
                        ...
                        >
                          <div
                            style={{
                              ...
                              fontSize: 18,
                              fontWeight: 900,
                              color: colorForPct(u.pct, thresholds),
                            }}
                          >
                            {fmtPct(u.pct)}
                          </div>
                          <div style={{ fontSize: 11, color: "var(--muted)" }}>
                            {(u.evidence || []).length} evidencias
                          </div>
                        </div>
                        <ProgressBar
                          value={u.pct}
            ...
            {drawerTab === "prescripcion" && (
              ...
              >
                ...
                >
                  Este estudiante requiere intervención prioritaria.
                </div>
                {drawerPrescription.map((p) => (
                  <Card
                    key={p.routeId}
                    title={p.title}
                    right={<span className="tag">{p.routeId}</span>}
                  >
                    {p.successCriteria && (
                      <div
                        style={{
            ...
            {drawerTab === "calidad" && (
              ...
              >
                ...
                >
                  Flags generados por el motor de calidad del gemelo. Indican
                  posibles inconsistencias en rúbricas, criterios no mapeados, o
                  datos ausentes.
                </div>
            ...
            {drawerTab === "historial" && (
              ...
              >
                <Card
                  title={`Historial de interacciones (${studentChatHook.entries.length})`}
                >
                  <div
                    style={{
                      fontSize: 12,
                  ...
                  >
                    Registra tus interacciones con este estudiante: reuniones,
                    emails, acciones tomadas. Se guarda localmente en tu
                    navegador.
                  </div>
                      ...
                      {studentChatHook.entries.map((entry) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <button
                              onClick={() =>
                                studentChatHook.deleteEntry(entry.id)
                              }
            ...
            {drawerTab === "notas" && (
              ...
              >
                <Card
                  ...
                  right={
                    studentNotesHook.lastUpdated ? (
                      ...
                      >
                        Guardado:{" "}
                        {new Date(studentNotesHook.lastUpdated).toLocaleString(
                          "es-CO",
                        )}
                      </span>
                    ) : null
                ...
                >
                  ...
                  >
                    Estas notas son <strong>privadas y locales</strong> a tu
                    navegador. No se comparten con el estudiante ni con otros
                    docentes. Útil para registrar observaciones, acuerdos,
                    recordatorios.
                  </div>
                  <textarea
                    ...
                    >
                      <button
                        onClick={() => {
                          if (
                            window.confirm(
                              "¿Borrar estas notas definitivamente?",
                            )
                          ) {
      ...
      {impersonateStudent && (
        ...
        >
          ...
          >
            <span>
              {" "}
              Vista previa: estás viendo como{" "}
              <strong>{impersonateStudent.name}</strong> (ID{" "}
              {impersonateStudent.userId})
            </span>
            <button
