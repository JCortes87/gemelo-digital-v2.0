    ...
    (async () => {
      try {
        ...
        const [userRes, coursesRes] = await Promise.allSettled([
          ...
          apiGet(
            `/brightspace/courses/enrolled?user_id=${userId}&active_only=true&limit=200${period ? `&period=${encodeURIComponent(period)}` : ""}`,
          ),
        ]);
        ...
        if (coursesRes.status === "fulfilled") {
          const items = Array.isArray(coursesRes.value?.items)
            ? coursesRes.value.items
            : [];
          setCourses(items);
        } else {
          throw (
            coursesRes.reason || new Error("No se pudieron cargar los cursos")
          );
        }
      } catch (e) {
      ...
      await mapLimit(courses, 5, async (course) => {
        ...
        try {
          const data = await apiGet(
            `/gemelo/course/${courseId}/student/${userId}`,
          );
          if (!alive) return;
  ...
  const kpis = useMemo(() => {
    ...
    const avgCoverage =
      withCoverage.length > 0
        ...
        : null;
    const atRisk = loaded.filter(
      (c) => c.risk === "alto" || c.risk === "medio",
    ).length;
    return {
          ...
          <>
            ...
            >
              <KpiCard
                ...
                sub={
                  loadingMetrics
                    ? `${kpis.loadedCourses} cargados...`
                    : `${kpis.loadedCourses} con datos`
                }
              />
              <KpiCard
                label="Promedio General"
                value={
                  kpis.avgGrade != null ? (kpis.avgGrade / 10).toFixed(1) : "—"
                }
                sub={kpis.avgGrade != null ? `/10` : "sin datos"}
                valueColor={
                  kpis.avgGrade != null
                    ? colorForPct(kpis.avgGrade)
                    : "var(--muted)"
                }
              />
              <KpiCard
                ...
                value={
                  kpis.avgCoverage != null
                    ? `${kpis.avgCoverage.toFixed(1)}%`
                    : "—"
                }
                sub="ítems calificados"
                valueColor={
                  kpis.avgCoverage != null
                    ? colorForPct(kpis.avgCoverage)
                    : "var(--muted)"
                }
              />
              <KpiCard
            ...
            {loadingMetrics && kpis.totalCourses > 0 && (
              ...
              >
                ...
                >
                  <span>
                    Analizando curso {kpis.loadedCourses} de {kpis.totalCourses}
                  </span>
                  <span>
                    {Math.round((kpis.loadedCourses / kpis.totalCourses) * 100)}
                    %
                  </span>
                </div>
                <div
                ...
                >
                  ...
                  <div style={{ width: "100%", height: 180 }}>
                    <ResponsiveContainer>
                      <LineChart
                        data={data}
                        margin={{ top: 6, right: 18, left: 0, bottom: 4 }}
                      >
                        <CartesianGrid
                          stroke="var(--border)"
                          strokeDasharray="3 3"
                        ...
                        <Tooltip
                          ...
                          formatter={(v, _n, p) => [
                            `${v} /10 (${p.payload.count} curso${p.payload.count !== 1 ? "s" : ""})`,
                            "Promedio",
                          ]}
                        />
                        <ReferenceLine
            ...
            >
              ...
              <button
                onClick={() => {
                  ...
                  const safeName = (userInfo?.name || `usuario_${userId}`)
                    .replace(/[^\w]+/g, "_")
                    .slice(0, 40);
                  downloadCsv(
                    `gemelo_estudiante_${safeName}_${ts}.csv`,
                    headers,
                    rows,
                  );
                }}
                ...
                style={{
                  ...
                  fontWeight: 800,
                  cursor:
                    filteredCourses.length === 0 ? "not-allowed" : "pointer",
            ...
            {filteredCourses.length === 0 ? (
              ...
              >
                <div
                  style={{ fontSize: 32, opacity: 0.4, marginBottom: 8 }}
                ></div>
                {courses.length === 0
                ...
                >
                  <thead>
                    <tr style={{ background: "var(--bg)" }}>
                      ...
                      <th
                        style={{ ...thStyle, textAlign: "center", width: 90 }}
                      >
                        Estado
                      </th>
                      <th
            ...
            >
              {filteredCourses.length} de {courses.length} curso
              {courses.length !== 1 ? "s" : ""}
