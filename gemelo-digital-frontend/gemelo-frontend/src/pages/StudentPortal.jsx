    ...
    >
      <div
        style={{
          ...
          borderRadius: 999,
          transition: animate ? "width 0.7s cubic-bezier(.4,0,.2,1)" : "none",
        }}
      />
    </div>
  );
...
export default function StudentPortal({
    ...
    (async () => {
      ...
      try {
        const data = await apiGet(
          "/brightspace/courses/enrolled?active_only=false&limit=200",
        );
        if (!alive) return;
  ...
  const impersonateUser = !userIdOverride
    ? sessionStorage.getItem("gemelo_impersonate_user")
    : null;
  const userId =
    userIdOverride ||
    (impersonateUser ? Number(impersonateUser) : null) ||
    authUser?.user_id;
  ...
  const courseName = useMemo(() => {
    ...
    const match = studentCourses.find(
      (c) =>
        String(c.id) === String(orgUnitId) ||
        String(c.orgUnitId) === String(orgUnitId),
    );
    ...
    (async () => {
      try {
        const [studentRes, courseRes, loRes] = await Promise.allSettled([
          apiGet(`/gemelo/course/${orgUnitId}/student/${userId}`, {
  ...
  const macroUnits = (
    studentData?.macroUnits ||
    studentData?.macro?.units ||
    []
  ).map((u) => ({
    code: u.code,
    pct: Number(u.pct || 0),
  ...
  const qualityFlags = Array.isArray(studentData?.qualityFlags)
    ? studentData.qualityFlags.filter(
        (f) => f?.type && f.type !== "role_not_enabled",
      )
    : [];
  ...
  const outcomesMap = useMemo(() => {
    const sets = Array.isArray(learningOutcomesPayload?.outcomeSets)
      ? learningOutcomesPayload.outcomeSets
      : [];
    const map = {};
    ...
    return (
      ...
      >
        ...
        >
          ...
          >
            Hola, {firstName}. Para ver tu información académica, accede desde
            tu curso en Brightspace usando el enlace de G.D.
          </p>
          <button
            onClick={logout}
            style={{
      ...
      >
        ...
        >
          ...
          <div style={{ fontSize: 13, color: "var(--muted)", marginTop: 4 }}>
            Consolidando tu gemelo digital
          </div>
        </div>
      </div>
    );
    ...
    >
          ...
          >
            ...
            >
              <div>
                ...
                >
                  {studentCourses.length} curso
                  {studentCourses.length !== 1 ? "s" : ""} como estudiante
                </div>
              </div>
              <button
                onClick={() => setShowCoursePanel(false)}
                  ...
                  {studentCourses.map((c) => (
                    <button
                      ...
                      style={{
                        ...
                        background:
                          c.id === orgUnitId ? "var(--ok-bg)" : "var(--bg)",
                        cursor: "pointer",
                        textAlign: "left",
                        width: "100%",
                    ...
                    >
                      ...
                      <span
                        style={{
                          color:
                            c.id === orgUnitId ? "var(--ok)" : "var(--muted)",
                          fontSize: 16,
                          flexShrink: 0,
                        }}
                      >
      ...
      >
        ...
        >
          ...
          >
            ...
            <div style={{ fontSize: 10, color: "var(--muted)" }}>
              {summary?.gradedItemsCount ?? 0}/{summary?.totalItemsCount ?? 0}{" "}
              ítems
            </div>
          </div>
          ...
          >
            ...
            {overdueUnscoredPct > 0 && (
              ...
              >
                ...
                >
                  {fmtPct(overdueUnscoredPct)}
                </span>
              </div>
            )}
            {pendingUngradedPct === 0 && overdueUnscoredPct === 0 && (
              <div
                style={{ fontSize: 12, color: "var(--ok)", fontWeight: 700 }}
              >
                {" "}
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            >
              ...
              <div
                style={{
                  background:
                    "linear-gradient(135deg, var(--brand) 0%, var(--brand-2, #003EA6) 100%)",
                  borderRadius: 12,
                  padding: 16,
                  color: "#fff",
                }}
                ...
                >
                  {(route.actions || []).map((a, i) => (
                    ...
                    >
                      ...
                      <span
                        style={{ fontSize: 13, lineHeight: 1.4, opacity: 0.95 }}
                      >
                        {a}
                      </span>
                    </div>
          ...
          return (
            <div style={{ marginBottom: 20 }}>
              <Card title="Resumen por Cortes" accent="brand">
                ...
                >
                  Cada sección muestra sus{" "}
                  <strong>ponderados acumulados</strong> y las evidencias que la
                  componen. Los resúmenes agregados{" "}
                  <strong>no cuentan dos veces</strong> en tu promedio.
                </div>
                  ...
                  return (
                    ...
                    >
                      {corteGroups.map((g, gi) => {
                        ...
                        const mainCorte =
                          corteList.find((e) => e.scorePct != null) ||
                          corteList[0];
                        const headerPct = mainCorte?.scorePct;
                        const headerColor =
                          headerPct != null
                            ? colorForPct(headerPct, thresholds)
                            : "var(--muted)";
                        const label = g.name;
                          ...
                          >
                            ...
                            <div style={{ padding: "12px 16px" }}>
                                ...
                                >
                                  {corteList
                                    ...
                                    .map((c, idx) => {
                                      const col =
                                        c.scorePct != null
                                          ? colorForPct(c.scorePct, thresholds)
                                          : "var(--muted)";
                                      return (
                                        <span
                                          key={`c-${idx}`}
                              ...
                              {evList.length > 0 ? (
                                <div>
                                  ...
                                  >
                                    {evList.map((e, idx) => {
                                      const col =
                                        e.scorePct != null
                                          ? colorForPct(e.scorePct, thresholds)
                                          : "var(--muted)";
                                ...
                                >
                                  Sin evidencias individuales detectadas en este
                                  corte
                                </div>
                              )}
        ...
        {overdueItems.length > 0 && (
          <div style={{ marginBottom: 20 }}>
            <Card
              title={` Entregas Vencidas (${overdueItems.length})`}
              accent="critical"
            >
              <div
                style={{
              ...
              >
                Estas evidencias ya pasaron su fecha de entrega y no han sido
                calificadas. Contacta a tu docente lo antes posible.
              </div>
        ...
        {pendingItems.length > 0 && (
          <div style={{ marginBottom: 20 }}>
            <Card title={`⏳ Entregas Pendientes (${pendingItems.length})`}>
              ...
              >
                Evidencias que aún puedes entregar. Revisa las fechas de
                vencimiento.
              </div>
        ...
        {gradedItems.length > 0 && (
          <div style={{ marginBottom: 20 }}>
            <Card
              title={`Mis Evidencias Calificadas (${gradedItems.length})`}
              accent="brand"
            >
              {/* Chart */}
              {chartData.length > 1 && (
                      ...
                      >
                        ...
                        <td
                          style={{
                            ...
                            fontSize: 14,
                            fontWeight: 900,
                            color: colorForPct(e.scorePct, thresholds),
                          }}
                        >
                          {e.scorePct != null
        ...
        {orgUnitId ? (
          <div style={{ marginBottom: 20 }}>
            <Card title=" Mis próximas entregas">
              ...
              >
                Aquí ves todas las entregas del curso organizadas por fecha.
                Pasa el cursor sobre una tarea para ver cuántos días te quedan y
                el rango completo de fechas disponible para entregar. Las
                entregas marcadas con{" "}
                <strong style={{ color: "#dc2626" }}>¡!</strong> vencen en menos
                de 2 días.
              </div>
              <DueDateCalendar
                orgUnitId={orgUnitId}
        ...
        {projection &&
          ...
          projection.scenarios.length > 0 && (
            <div style={{ marginBottom: 20 }}>
              <Card
                title="Proyección de mi Nota Final"
                right={
                  <span className="tag">
                    {fmtPct(projection.coveragePct)} calificado
                  </span>
                }
              >
                {projection.isFinal ? (
                  <div>
                    ...
                    <div
                      style={{
                        fontSize: 36,
                        fontWeight: 900,
                        color: colorForPct(projection.finalPct, thresholds),
                        fontFamily: "var(--font-mono)",
                      }}
                    >
                      {fmtGrade10FromPct(projection.finalPct)}
                    </div>
                    <div
                      style={{
                        fontSize: 12,
                    ...
                    >
                      Cobertura 100% — esta es tu nota definitiva.
                    </div>
                  </div>
                ) : (
                  <>
                    ...
                    >
                      ...
                      Tu nota final depende de lo que ya tienes calificado (
                      {fmtPct(projection.coveragePct)}) y de cómo te vaya en lo
                      que queda pendiente. Usamos 3 escenarios para darte una
                      idea del rango:
                      <ul
                        style={{
                          margin: "6px 0 0 0",
                      ...
                      >
                        <li>
                          ...
                          supone que el resto de evidencias las entregas con un
                          desempeño inferior al actual — es tu "piso" si te
                          relajas.
                        </li>
                        <li>
                          ...
                          supone que mantienes el mismo nivel que llevas hasta
                          ahora en lo que falta.
                        </li>
                        <li>
                          ...
                          supone que das lo mejor en las evidencias pendientes —
                          es tu "techo" si te esfuerzas más.
                        </li>
                      </ul>
                      <div
                        style={{
                      ...
                      >
                        Los tres valores cambian a medida que tus docentes
                        califiquen más evidencias. Entre más se acerque la
                        cobertura al 100%, más precisa será la proyección.
                      </div>
                    </div>
                    ...
                    >
                      {projection.scenarios.map((s) => {
                        ...
                        return (
                          <div
                            key={s.id}
                            className={`scenario-card ${meta.cls}`}
                          >
        ...
        {prescription.length > 0 && (
          <div style={{ marginBottom: 20 }}>
            <Card title="Plan de Intervención" accent="watch">
              ...
              >
                Tu docente ha creado un plan de intervención personalizado para
                ti.
              </div>
              {prescription.map((p) => (
                <div
        ...
        {qualityFlags.length > 0 && (
          <div style={{ marginBottom: 20 }}>
            <Card title="Calidad de Datos">
              ...
              >
                Algunos indicadores pueden estar incompletos debido a datos
                faltantes en la configuración del curso.
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
