...
export function exportStudentsCsv(studentRows, courseInfo, options = {}) {
  ...
  const courseSafe = String(courseInfo?.Name || "curso")
    .replace(/[^\w]+/g, "_")
    .slice(0, 40);
  const filterSafe = filterDescription
    ? `_${String(filterDescription)
        .replace(/[^\w]+/g, "_")
        .slice(0, 30)}`
    : "";
...
<body>
 ...
 setTimeout(() => { window.print(); }, 400);
  ...
  if (!w) {
    console.warn(
      "No se pudo abrir la ventana de reporte. Revisa tu bloqueador de pop-ups.",
    );
    return;
  }
...
export function exportInstitutionalFeedbackPdf({
  ...
  const submittedAt = feedback.submittedAt
    ? new Date(feedback.submittedAt).toLocaleDateString("es-CO", {
        year: "numeric",
        month: "long",
        day: "numeric",
      })
  ...
  const grade10 =
    score != null && outOf != null && outOf > 0
      ? ((score / outOf) * 10).toFixed(1)
      : null;
  const pct =
    score != null && outOf != null && outOf > 0
      ? Math.min(100, (score / outOf) * 100)
      : null;
  ...
  const rubricsHtml = rubrics
    .map((r) => {
      const rPct =
        r.score != null && r.outOf != null && r.outOf > 0
          ? Math.min(100, (r.score / r.outOf) * 100)
          : null;
      const rColor = levelColorHex(r.level);
      const criteriaHtml = (Array.isArray(r.criteria) ? r.criteria : [])
        .map((c) => {
 ...
 <tbody>${criteriaHtml}</tbody>
     ...
     : `<div style="padding:14px;font-size:11px;color:#5A6580;font-style:italic;text-align:center">Rúbrica sin criterios evaluados.</div>`
 }
 </div>
 `;
    })
...
<body>
 ...
 if (document.readyState === "complete") { startPrint(); }
  ...
  if (!w) {
    alert(
      "No se pudo abrir la ventana del reporte. Revisa tu bloqueador de pop-ups.",
    );
    return;
  }
  w.document.write(html);
